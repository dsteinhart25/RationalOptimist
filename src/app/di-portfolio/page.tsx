import portfolioData from "../../data/di-portfolio.json";

const BASE = "/RationalOptimist";

/* ── Disruption Investor brand colors ── */
const DI = {
...
function GainLoss({ value }: { value: number }) {
  const color = value > 0 ? "#34d399" : value < 0 ? "#f87171" : DI.textMuted;
  return (
...
/* ── Main page ── */
export default function DIPortfolioPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: DI.bg, color: DI.textPrimary }}>
      {/* ─── Navigation ─── */}
      <nav
        className="sticky top-0 z-50 backdrop-blur-xl"
        style={{ background: `${DI.bg}ee`, borderBottom: `1px solid ${DI.border}` }}
      >
        <div className="flex items-center justify-between px-6 py-3 max-w-[1400px] mx-auto">
          <a href={`${BASE}/di-portfolio`} className="flex items-center gap-3">
            <div
              className="h-8 w-8 rounded-lg flex items-center justify-center font-bold text-sm"
              style={{ background: DI.accentDim, color: DI.accent, border: `1px solid ${DI.accent}33` }}
            >
              DI
            </div>
            <div className="flex flex-col">
              ...
              >
                <span style={{ color: DI.textPrimary }}>Disruption</span>{" "}
                <span style={{ color: DI.accent }}>Investor</span>
              </span>
              <span
                className="text-[10px] font-medium uppercase tracking-[0.15em] leading-tight"
                style={{ color: DI.textMuted }}
              >
                RiskHedge
              </span>
            </div>
      ...
      <main
        ...
        style={{
          background: `linear-gradient(180deg, ${DI.bg} 0%, #0f1729 50%, ${DI.bg} 100%)`,
        }}
      >
        <div className="max-w-[1400px] mx-auto">
          ...
          <div className="mb-10">
            <div className="flex items-center gap-3 mb-3">
              <h1
                ...
                style={{ fontFamily: "var(--font-display)" }}
              >
                <span style={{ color: DI.accent }}>Disruption Investor</span>{" "}
                <span style={{ color: DI.textPrimary }}>Portfolio</span>
              </h1>
            </div>
            <p className="text-sm" style={{ color: DI.textSecondary }}>
              Active positions and guidance from the Disruption Investor advisory.
              {lastUpdated && (
                <span style={{ color: DI.textMuted }} className="ml-2">
            ...
            <div className="mb-12">
              <div className="flex items-center gap-3 mb-4">
                <h2
                  ...
                  style={{ fontFamily: "var(--font-display)", color: DI.textPrimary }}
                >
                  Open Positions
                </h2>
                <span
                  className="text-xs px-2 py-0.5 rounded-full font-medium"
                  style={{ background: DI.accentDim, color: DI.accent }}
                >
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr
                        ...
                        style={{ background: "#151d2e", color: DI.textMuted }}
                      >
                      ...
                      {positions.map((p, i) => {
                        ...
                        return (
                          <tr
                            key={p.id}
                            style={{
                              background: i % 2 === 0 ? DI.bgCard : "#0f1729",
                              borderBottom: `1px solid ${DI.border}`,
                            }}
                          >
                            <td className="px-4 py-3 font-semibold whitespace-nowrap" style={{ color: DI.accent }}>
                              {p.ticker}
                            </td>
                            <td className="px-4 py-3 whitespace-nowrap" style={{ color: DI.textPrimary }}>
                              {p.company}
                            </td>
                            <td className="px-4 py-3">
                              <ActionBadge action={p.action} />
                            </td>
                            <td className="px-4 py-3 text-xs" style={{ color: DI.textSecondary }}>
                              {p.positionSize}
                            </td>
                            <td className="px-4 py-3 text-right font-mono" style={{ color: DI.textPrimary }}>
                            ...
                            <td className="px-4 py-3 whitespace-nowrap" style={{ color: DI.textSecondary }}>
                              {fmtShort(p.buyDate)}
                            </td>
                            <td className="px-4 py-3 whitespace-nowrap" style={{ color: DI.textSecondary }}>
                              {p.freeRideDate ? (
                            ...
                            <td className="px-4 py-3 text-right whitespace-nowrap" style={{ color: DI.textMuted }}>
                            ...
                            <td className="px-4 py-3 text-xs max-w-[220px]" style={{ color: DI.textMuted }}>
                              {p.guidance || p.notes || "\u2014"}
                            </td>
                          </tr>
                        );
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-base" style={{ color: DI.accent }}>
                            {p.ticker}
                          </span>
                          <span className="text-sm" style={{ color: DI.textSecondary }}>
                            {p.company}
                          </span>
                        </div>
                        <ActionBadge action={p.action} />
                      ...
                      <div className="flex items-center justify-between mb-3 px-1">
                        <div>
                          <span className="text-xs block" style={{ color: DI.textMuted }}>Entry</span>
                          <span className="font-mono text-sm" style={{ color: DI.textPrimary }}>
                            ${p.buyPrice.toFixed(2)}
                          </span>
                        </div>
                        <div className="text-center">
                          <span className="text-xs block" style={{ color: DI.textMuted }}>Latest</span>
                          {p.latestPrice !== null ? (
                            <span
                            ...
                            <span className="text-sm" style={{ color: DI.textMuted }}>{"\u2014"}</span>
                          )}
                        </div>
                        <div className="text-right">
                          <span className="text-xs block" style={{ color: DI.textMuted }}>Return</span>
                          {totalReturn !== null ? (
                      ...
                      <div
                        ...
                        style={{ borderTop: `1px solid ${DI.border}` }}
                      >
                        <div>
                          <span style={{ color: DI.textMuted }}>Buy Date</span>
                          <p style={{ color: DI.textSecondary }}>{fmtShort(p.buyDate)}</p>
                        </div>
                        <div>
                          <span style={{ color: DI.textMuted }}>Holding</span>
                          <p style={{ color: DI.textSecondary }}>
                            {holdingPeriod(p.buyDate, p.sellDate)}
                          </p>
                        </div>
                        <div>
                          <span style={{ color: DI.textMuted }}>Size</span>
                          <p style={{ color: DI.textSecondary }}>{p.positionSize}</p>
                        </div>
                        {p.freeRideDate && (
                          <div className="col-span-2">
                            <span style={{ color: DI.textMuted }}>Free Ride</span>
                        ...
                        {p.stopLoss !== null && (
                          <div>
                            <span style={{ color: DI.textMuted }}>Hard Stop</span>
                      ...
                      {/* Guidance */}
                      {(p.guidance || p.notes) && (
                        <div className="mt-3 pt-2" style={{ borderTop: `1px solid ${DI.border}` }}>
                          {p.guidance && (
                            <p className="text-xs" style={{ color: DI.textMuted }}>{p.guidance}</p>
                          )}
                          {p.notes && (
                            <p className="text-xs mt-1 italic" style={{ color: DI.textMuted }}>{p.notes}</p>
                          )}
                        </div>
                      )}
                    </div>
          ...
          {positions.length === 0 && (
            ...
            >
              ...
              <p className="text-lg font-semibold mb-2" style={{ color: DI.textPrimary }}>
                No positions tracked yet
              </p>
              <p className="text-sm" style={{ color: DI.textMuted }}>
                Disruption Investor positions will appear here once added.
              </p>
            </div>
          )}
        </div>
      ...
      {/* ─── Footer ─── */}
      <footer style={{ borderTop: `1px solid ${DI.border}`, background: DI.bg }} className="py-8">
        <div className="max-w-[1400px] mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <span
              ...
              style={{ fontFamily: "var(--font-display)" }}
            >
              <span style={{ color: DI.textPrimary }}>Risk</span>
              <span style={{ color: DI.accent }}>Hedge</span>
            </span>
            <span className="text-xs" style={{ color: DI.textMuted }}>
              Disruption Investor
            </span>
          </div>
          <p className="text-sm" style={{ color: DI.textMuted }}>
